<template>
  <section class="todoapp">
    <!-- header -->
    <header class="header">
      <input class="new-todo" autocomplete="off" placeholder="type todo List here" @keyup.enter="addTodo">
    </header>
    <!-- main section -->
    ...
    <footer class="footer" v-show="todos.length">
      ...
      <ul class="filters">
        <li v-for="(val, key) in filters" :key="key">
          <a :class="{ selected: visibility === key }" @click.prevent="visibility = val">{{ key | capitalize }}</a>
        </li>
      </ul>
    </footer>
  </section>
